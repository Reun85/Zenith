...
unsafe extern "system" fn vulkan_debug_callback(
    ...
    match message_severity {
        Severity::INFO => {
            zenith::log::info!("{:?}: [{}] ({})", message_type, message_id_name, message)
        }
        Severity::VERBOSE => {
            zenith::log::trace!("{:?}: [{}] ({})", message_type, message_id_name, message)
        }
        Severity::WARNING => {
            zenith::log::warn!("{:?}: [{}] ({})", message_type, message_id_name, message)
        }
        Severity::ERROR => {
            zenith::log::error!("{:?}: [{}] ({})", message_type, message_id_name, message)
        }
        _ => zenith::log::error!("{:?}: [{}] ({})", message_type, message_id_name, message),
    }
...
impl ExampleBase {
    ...
    pub fn new(window_width: u32, window_height: u32) -> Self {
        ...
        let layers_names_raw: Vec<*const c_char> = layer_names
            .iter()
            .map(|raw_name| raw_name.as_ref().as_ptr())
            .collect();
        ...
        //This is unsafe since we have to make sure this lives throughout our use of the vulkan
        //api? thats dumb
