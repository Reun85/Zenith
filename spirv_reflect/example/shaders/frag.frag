...
layout(push_constant) uniform PushConstants {
		vec4 lightPos[MAX_LIGHT_COUNT];
		vec3 La[MAX_LIGHT_COUNT];
		vec3 Ld[MAX_LIGHT_COUNT];
		vec3 Ls[MAX_LIGHT_COUNT];
		float lightConstantAttenuation[MAX_LIGHT_COUNT];
		float lightLinearAttenuation[MAX_LIGHT_COUNT];
		float lightQuadraticAttenuation[MAX_LIGHT_COUNT];
		int lightCount;
} pushConstants;
